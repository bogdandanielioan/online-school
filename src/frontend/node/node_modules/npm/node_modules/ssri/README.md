...
// your PrivateRouteCourseDetails.js file so you can use a single integrity string and serve
...
```javascript
ssri.fromStream(fs.createReadStream('PrivateRouteCourseDetails.js'), {
  algorithms: ['sha1', 'sha512']
}).then(integrity => {
  return ssri.checkStream(fs.createReadStream('PrivateRouteCourseDetails.js'), integrity)
}) // succeeds
```
...
```javascript
const data = fs.readFileSync('PrivateRouteCourseDetails.js')
...
```javascript
const integrity = ssri.fromData(fs.readFileSync('PrivateRouteCourseDetails.js'))

ssri.checkStream(
  fs.createReadStream('PrivateRouteCourseDetails.js'),
  integrity
)
// ->
// Promise<{
